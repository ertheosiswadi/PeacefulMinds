...
class ResultsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.global(qos: .background).async{
            self.apihelper.getRequest(username: self.username!, procedure: "procedure_2"){ (result) in
...
//        tableScrollView.contentSize = CGSize(width: Int(tableScrollView.frame.size.width), height: 20 + n_hospitals * (tb_height + spacing))
    ...
    func add_hospital(hospital_json:[String:Any], n_ExistingHospitals:Int)
    {
        ...
        add_hname(name: "Mount Sinai", table: myTableView)//add_hname(name: "Hospital_\(n_ExistingHospitals)", table: myTableView)
...
//        row.addSubview(label_4)
    ...
    func add_hname(name: String, table:UIView)
    {
        let lb_width = 350;
        let lb_height = 80;
        let container = UIView(frame: CGRect(x: 0, y: 0, width: lb_width, height: lb_height))
        let title = UILabel(frame: CGRect(x: 20, y: 10, width: lb_width-10, height: lb_height-10))
        
        container.roundCorners_v1(cornerRadius: 30.0)
        container.backgroundColor = UIColor.white
        title.text = name;
        title.font = UIFont.boldSystemFont(ofSize: 50.0)
        title.font = UIFont(name: "Montserrat-SemiBold", size: 30)
